...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
  ...
  # version du serveur
  server_version = 'INFO_TC3/server_project.py/0.1'
 ...
 # méthode pour traiter les requêtes POST
      ...
      if self.path_info[1] == "scene":
        ...
        if ( len(self.path_info) < 3 or self.path_info[2] == ''):
          ...
          if 'width' in info and not int(info['width']) == 0 and \
            ...
            scene = self.create_image(info['name'],info['width'], info['height'], info['serial'], filename, obj=None)
        ...
        # POST /service/scene/%id
          ...
          if "operation" in info and info['operation'] == "update":
            ...
            if not data:
              ...
              if len(data) == 6 :
                  return
                  (name, width, height, serial, filename, objet) = [k for k in data]
              else:
                  data.append(None)
                  return 
                  (name, width, height, serial, filename,objet) = [k for k in data]
            ...
            if (('width' in info and info['width'] and not info['width'] == width) or \
              ...
              try:
                ...
                scene = self.create_image( name,
                  ...
                  info['filename'], info['objet'] if 'objet' in info else objet )
  ...
  def create_image(self, name, width, height, serial, filename, obj):
    if not filename:
...
conn = sqlite3.connect('bdserver.sqlite')
